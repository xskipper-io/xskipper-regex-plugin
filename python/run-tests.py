...
def test(root_dir, package):
    ...
    test_dir = path.join(python_root_dir, path.join("regexplugin", "tests"))
    test_files = [os.path.join(test_dir, f) for f in os.listdir(test_dir)
                  if os.path.isfile(os.path.join(test_dir, f)) and
                  f.endswith(".py") and not f.startswith("_")]
    extra_class_path = path.join(python_root_dir, path.join("regexplugin", "testing"))

    with open(os.path.join(root_dir, "python", "xskipper-version.txt")) as fd:
        xskipper_package = fd.readline()

    for test_file in test_files:
        try:
            cmd = ["spark-submit",
                   "--driver-class-path=%s" % extra_class_path,
                   "--packages", "{0},{1}".format(package, xskipper_package), test_file]
...
def prepare(root_dir):
    ...
    delete_if_exists(os.path.expanduser("~/.ivy2/cache/io.xskipper/xskipper-regex-plugin_2.12"))
    delete_if_exists(os.path.expanduser("~/.m2/repository/io/xskipper/xskipper-regex-plugin_2.12"))
    ...
    package = "io.xskipper:xskipper-regex-plugin_2.12:" + version
    return package
